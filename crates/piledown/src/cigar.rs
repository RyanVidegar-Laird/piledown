...
mod tests {
    ...
    fn anchor_zero_returns_unchanged() {
        let spans = vec![
            CigarSpan::Match { start: 100, len: 10 },
            CigarSpan::Skip { start: 110, len: 500 },
            CigarSpan::Match { start: 610, len: 15 },
        ];
    ...
    fn anchor_no_skips_unchanged() {
        let spans = vec![CigarSpan::Match { start: 100, len: 50 }];
    ...
    fn anchor_single_junction_left_fails() {
        ...
        let spans = vec![
            CigarSpan::Match { start: 100, len: 3 },
            CigarSpan::Skip { start: 103, len: 500 },
            CigarSpan::Match { start: 603, len: 15 },
        ];
        let expected = vec![CigarSpan::Match { start: 603, len: 15 }];
    ...
    fn anchor_single_junction_right_fails() {
        ...
        let spans = vec![
            CigarSpan::Match { start: 100, len: 15 },
            CigarSpan::Skip { start: 115, len: 500 },
            CigarSpan::Match { start: 615, len: 3 },
        ];
        let expected = vec![CigarSpan::Match { start: 100, len: 15 }];
        assert_eq!(filter_spans_by_anchor(&spans, 5), expected);
    }
    ...
    fn anchor_multi_junction_middle_fails() {
        ...
        let spans = vec![
            CigarSpan::Match { start: 100, len: 30 },
            CigarSpan::Skip { start: 130, len: 500 },
            CigarSpan::Match { start: 630, len: 3 },
            CigarSpan::Skip { start: 633, len: 200 },
            CigarSpan::Match { start: 833, len: 30 },
        ];
        let expected = vec![
            CigarSpan::Match { start: 100, len: 30 },
            CigarSpan::Match { start: 833, len: 30 },
    ...
    fn anchor_adjacent_skips_both_fail() {
        ...
        let spans = vec![
            CigarSpan::Match { start: 100, len: 10 },
            CigarSpan::Skip { start: 110, len: 100 },
            CigarSpan::Skip { start: 210, len: 200 },
            CigarSpan::Match { start: 410, len: 10 },
        ];
        let expected = vec![
            CigarSpan::Match { start: 100, len: 10 },
            CigarSpan::Match { start: 410, len: 10 },
    ...
    fn anchor_skip_at_start_fails() {
        let spans = vec![
            CigarSpan::Skip { start: 100, len: 500 },
            CigarSpan::Match { start: 600, len: 20 },
        ];
        let expected = vec![CigarSpan::Match { start: 600, len: 20 }];
        assert_eq!(filter_spans_by_anchor(&spans, 5), expected);
    }
    ...
    fn anchor_skip_at_end_fails() {
        let spans = vec![
            CigarSpan::Match { start: 100, len: 20 },
            CigarSpan::Skip { start: 120, len: 500 },
        ];
        let expected = vec![CigarSpan::Match { start: 100, len: 20 }];
        assert_eq!(filter_spans_by_anchor(&spans, 5), expected);
    }
    ...
    fn anchor_deletion_with_long_flanks_passes() {
        ...
        let spans = vec![
            ...
            CigarSpan::Match { start: 122, len: 20 },
        ];
        assert_eq!(filter_spans_by_anchor(&spans, 5), spans);
    }
    ...
    fn anchor_insertion_splits_match_conservative() {
        ...
        let spans = vec![
            ...
            CigarSpan::Match { start: 105, len: 5 },
            CigarSpan::Skip { start: 110, len: 500 },
            CigarSpan::Match { start: 610, len: 10 },
        ];
        assert_eq!(filter_spans_by_anchor(&spans, 5), spans);
    }
    ...
    fn anchor_insertion_splits_match_too_short() {
        ...
        let spans = vec![
            ...
            CigarSpan::Skip { start: 106, len: 500 },
            CigarSpan::Match { start: 606, len: 10 },
        ];
        let expected = vec![
            CigarSpan::Match { start: 100, len: 3 },
            CigarSpan::Match { start: 606, len: 10 },
    ...
    fn anchor_exact_boundary_passes() {
        ...
        let spans = vec![
            CigarSpan::Match { start: 100, len: 5 },
            CigarSpan::Skip { start: 105, len: 500 },
            CigarSpan::Match { start: 605, len: 15 },
        ];
        assert_eq!(filter_spans_by_anchor(&spans, 5), spans);
    }
